...
function Toggle({ value, onChange }: { value: boolean; onChange: (v: boolean) => void }) {
  return (
    <button
      ...
      className={cn('rounded-full relative transition-colors duration-200 flex-shrink-0', value ? 'bg-indigo-600' : 'bg-slate-200')}
    >
      <motion.div
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-0.5">
          <span className="text-sm font-bold text-slate-900">{provider.label}</span>
        ...
        <p className="text-xs font-mono text-slate-500 truncate">{visible ? savedKey.value : masked}</p>
        {savedKey.savedAt && <p className="text-[10px] text-slate-400 mt-0.5">Salva em {savedKey.savedAt}</p>}
      </div>
      <button onClick={() => setVisible(!visible)} className="p-2 text-slate-400 hover:text-slate-600 rounded-lg transition-colors">
          ...
          <AnimatePresence mode="wait">
            ...
            {step === 'choose' && (
              <motion.div key="choose" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }}>
                {available.length === 0 ? (
                  <p className="text-sm text-slate-500 text-center py-4">Todos os provedores já estão configurados! ✅</p>
                ) : (
                    ...
                    {available.map(provider => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <div>
                            <p className="text-sm font-bold text-slate-900 group-hover:text-indigo-700 leading-tight">{provider.label}</p>
                            <p className="text-[11px] text-slate-500 leading-tight mt-0.5 line-clamp-2">{provider.description}</p>
                          </div>
                        </button>
                      );
                    })}
            ...
            {step === 'enter' && selectedProvider && (
              <motion.div key="enter" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} className="space-y-4">
                <div className="flex items-center gap-3 p-3 bg-slate-50 rounded-xl">
                  ...
                  <div>
                    <p className="text-sm font-bold text-slate-900">{selectedProvider.label}</p>
                    <p className="text-xs text-slate-500">{selectedProvider.description}</p>
                  </div>
                </div>

                <div>
                  <label className="text-[10px] font-bold uppercase text-slate-400 tracking-wider mb-1.5 block">Chave API</label>
                  <div className="relative">
                    <input
                      ...
                      autoFocus
                      className="w-full pl-3 pr-10 py-2.5 text-sm font-mono bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-300 text-slate-800 placeholder:font-sans placeholder:text-slate-400"
                    />
                    <button onClick={() => setVisible(!visible)} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600">
                      {visible ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                    </button>
                  </div>
          ...
          <div className="max-w-6xl mx-auto">
            ...
            <div className="flex items-center gap-4 mb-8">
              ...
              <div>
                ...
                <p className="text-slate-500 text-sm">Gerencie sua conta, integrações e preferências.</p>
              </div>
            </div>
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          ...
                          isActive ? 'bg-indigo-600 text-white shadow-md shadow-indigo-200' : 'text-slate-600 hover:bg-white hover:text-slate-900 hover:shadow-sm'
                        )}
                      >
                        <Icon className="w-4 h-4 flex-shrink-0" />
                        {section.label}
                        {section.id === 'api' && configuredKeys.length > 0 && (
                          <span className={cn('ml-auto text-[10px] font-bold px-1.5 py-0.5 rounded-full', isActive ? 'bg-white/20 text-white' : 'bg-emerald-100 text-emerald-700')}>
                ...
                <AnimatePresence mode="wait">
                  <motion.div key={activeSection} initial={{ opacity: 0, x: 8 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -8 }} transition={{ duration: 0.15 }}>
                    ...
                    {activeSection === 'api' && (
                      <div className="space-y-5">
                        <div className="flex items-start justify-between">
                          <div>
                            <h2 className="text-lg font-bold text-slate-900 mb-1">Chaves de API de IA</h2>
                            <p className="text-sm text-slate-500">Configure as chaves para ativar as Ferramentas de IA. As chaves são armazenadas localmente no seu navegador.</p>
                          </div>
                          <button
                        ...
                        <AnimatePresence>
                          {configuredProviders.length > 0 ? (
                            ...
                            <div className="bg-white rounded-2xl border-2 border-dashed border-slate-200 p-12 text-center">
                              ...
                              <p className="text-sm font-semibold text-slate-700 mb-1">Nenhuma chave configurada</p>
                              <p className="text-xs text-slate-500 mb-4">Adicione a chave de um provedor de IA para ativar as Ferramentas.</p>
                              <button
                                onClick={() => setShowAddKey(true)}
                        ...
                        {configuredKeys.length > 0 && configuredKeys.length < AI_PROVIDERS.length && (
                          <div>
                            <p className="text-[10px] font-bold uppercase text-slate-400 tracking-widest mb-3">Provedores disponíveis</p>
                            <div className="grid grid-cols-2 gap-3">
                              {AI_PROVIDERS.filter(p => !configuredKeys.find(k => k.id === p.id)).map(provider => {
                                ...
                                return (
                                  <button
                                    key={provider.id}
                                    onClick={() => setShowAddKey(true)}
                                    className="flex items-center gap-3 p-3 bg-white rounded-xl border border-slate-100 shadow-sm hover:shadow-md hover:border-indigo-200 transition-all text-left group"
                                  >
                                    <div className={`w-8 h-8 rounded-lg bg-gradient-to-br ${provider.color} flex items-center justify-center flex-shrink-0`}>
                                      <Icon className="w-4 h-4 text-white" />
                                    </div>
                                    <div className="flex-1 min-w-0">
                                      <p className="text-xs font-bold text-slate-700 group-hover:text-indigo-700 truncate">{provider.label}</p>
                                      <p className="text-[10px] text-slate-400">Não configurado</p>
                                    </div>
                                    <Plus className="w-3.5 h-3.5 text-slate-400 group-hover:text-indigo-500 flex-shrink-0" />
                                  </button>
                                );
                              })}
                            </div>
                    ...
                    {activeSection === 'profile' && (
                      <div className="space-y-5">
                        <div>
                          <h2 className="text-lg font-bold text-slate-900 mb-1">Perfil</h2>
                          <p className="text-sm text-slate-500">Informações da sua conta e identidade visual.</p>
                        </div>
                        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
                          <label className="text-[10px] font-bold uppercase text-slate-400 tracking-wider mb-4 block">Foto de Perfil</label>
                          <div className="flex items-center gap-5">
                            ...
                            <div>
                              <p className="text-sm text-slate-700 font-semibold mb-1">{settings.profile.name}</p>
                              <p className="text-xs text-slate-500 mb-2">{settings.profile.email}</p>
                              <button className="text-xs font-bold text-indigo-600 hover:underline">Alterar foto</button>
                            </div>
                          </div>
                        </div>
                        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 space-y-4">
                          <label className="text-[10px] font-bold uppercase text-slate-400 tracking-wider block">Informações Pessoais</label>
                          {[{ field: 'name', label: 'Nome completo', icon: User }, { field: 'email', label: 'E-mail', icon: Mail }, { field: 'role', label: 'Cargo', icon: Globe }].map(({ field, label, icon: Icon }) => (
                            <div key={field}>
                              <label className="text-xs font-semibold text-slate-500 mb-1 flex items-center gap-1.5"><Icon className="w-3 h-3" />{label}</label>
                              <input
                                ...
                                className="w-full px-3 py-2.5 text-sm bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/30 text-slate-800"
                              />
                            </div>
                          ))}
                    ...
                    {activeSection === 'notifications' && (
                      <div className="space-y-5">
                        <div>
                          <h2 className="text-lg font-bold text-slate-900 mb-1">Notificações</h2>
                          <p className="text-sm text-slate-500">Escolha como e quando ser notificado.</p>
                        </div>
                            ...
                            return (
                              <div key={item.id} className="flex items-center justify-between p-5">
                                <div className="flex items-start gap-3">
                                  <div className="w-8 h-8 bg-slate-50 rounded-lg flex items-center justify-center mt-0.5"><Icon className="w-4 h-4 text-slate-500" /></div>
                                  <div>
                    ...
                    {activeSection === 'appearance' && (
                      <div className="space-y-5">
                        <div>
                          <h2 className="text-lg font-bold text-slate-900 mb-1">Aparência</h2>
                          <p className="text-sm text-slate-500">Personalize a interface conforme sua preferência.</p>
                        </div>
                        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
                          <label className="text-[10px] font-bold uppercase text-slate-400 tracking-wider mb-4 block">Tema</label>
                          <div className="grid grid-cols-3 gap-3">
                            {[{ id: 'light', label: 'Claro', icon: Sun }, { id: 'dark', label: 'Escuro', icon: Moon }, { id: 'system', label: 'Sistema', icon: Monitor }].map(theme => {
                              ...
                              return (
                                <button key={theme.id} onClick={() => setSettings(prev => ({ ...prev, appearance: { ...prev.appearance, theme: theme.id as any } }))} className={cn('flex flex-col items-center gap-2 p-4 rounded-xl border-2 transition-all', isSelected ? 'border-indigo-500 bg-indigo-50 shadow-md shadow-indigo-100' : 'border-slate-100 bg-slate-50 hover:border-slate-200')}>
                                  <Icon className={cn('w-5 h-5', isSelected ? 'text-indigo-600' : 'text-slate-500')} />
                                  <span className={cn('text-xs font-bold', isSelected ? 'text-indigo-600' : 'text-slate-500')}>{theme.label}</span>
                        ...
                        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
                          <label className="text-[10px] font-bold uppercase text-slate-400 tracking-wider mb-3 block">Idioma</label>
                          <select value={settings.appearance.language} onChange={(e) => setSettings(prev => ({ ...prev, appearance: { ...prev.appearance, language: e.target.value } }))} className="w-full max-w-xs px-3 py-2.5 text-sm bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/30 text-slate-700">
                        ...
                        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 flex items-center justify-between">
                          <div>
                            <p className="text-sm font-semibold text-slate-900">Modo Compacto</p>
                            <p className="text-xs text-slate-500 mt-0.5">Reduz o espaçamento para exibir mais conteúdo na tela.</p>
                          </div>
                    ...
                    {activeSection === 'team' && (
                      <div className="space-y-5">
                        <div>
                          <h2 className="text-lg font-bold text-slate-900 mb-1">Equipe & Acesso</h2>
                          <p className="text-sm text-slate-500">Gerencie membros e permissões do workspace.</p>
                        </div>
                        {[{ name: 'Ana Silva', email: 'ana@marketflow.com', role: 'Editor', avatar: 'https://picsum.photos/seed/ana/100/100', active: true }, { name: 'Bruno Costa', email: 'bruno@marketflow.com', role: 'Visualizador', avatar: 'https://picsum.photos/seed/bruno/100/100', active: true }, { name: 'Carla Souza', email: 'carla@marketflow.com', role: 'Editor', avatar: 'https://picsum.photos/seed/carla/100/100', active: false }].map((member) => (
                          <div key={member.email} className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 flex items-center gap-4">
                            ...
                            <div className="flex-1">
                              <div className="flex items-center gap-2">
                                ...
                                <span className={cn('text-[10px] font-bold px-1.5 py-0.5 rounded-full', member.active ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500')}>{member.active ? 'Ativo' : 'Inativo'}</span>
                              </div>
                              <p className="text-xs text-slate-500">{member.email}</p>
                            </div>
                            <select className="text-xs font-medium text-slate-700 bg-slate-50 border border-slate-200 rounded-lg py-1.5 px-2 focus:outline-none"><option>Editor</option><option>Visualizador</option><option>Admin</option></select>
                        ...
                        <button className="w-full flex items-center justify-center gap-2 py-3 border-2 border-dashed border-slate-200 rounded-2xl text-sm font-bold text-slate-500 hover:border-indigo-300 hover:text-indigo-600 transition-all">+ Convidar membro</button>
                      </div>
                    )}
                    ...
                    {activeSection === 'security' && (
                      <div className="space-y-5">
                        <div>
                          <h2 className="text-lg font-bold text-slate-900 mb-1">Segurança</h2>
                          <p className="text-sm text-slate-500">Proteja sua conta com autenticação adicional.</p>
                        </div>
                        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 space-y-4">
                          <label className="text-[10px] font-bold uppercase text-slate-400 tracking-wider block">Alterar Senha</label>
                          {['Senha atual', 'Nova senha', 'Confirmar nova senha'].map((label, i) => (
                            <div key={i}>
                              <label className="text-xs font-semibold text-slate-500 mb-1 block">{label}</label>
                              <input type="password" placeholder="••••••••" className="w-full px-3 py-2.5 text-sm bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/30" />
                            </div>
                          ))}
                        ...
                        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 flex items-center justify-between">
                          <div>
                            <p className="text-sm font-semibold text-slate-900">Autenticação em dois fatores (2FA)</p>
                            <p className="text-xs text-slate-500 mt-0.5">Adicione uma camada extra de proteção à sua conta.</p>
                          </div>
