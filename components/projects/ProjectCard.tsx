...
const STATUS_STYLES: Record<Project['status'], string> = {
  ...
  Concluído: 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 dark:text-slate-500',
};
...
export function ProjectCard({ project, onClick, onEdit, onDelete }: ProjectCardProps) {
  ...
  return (
    <div
      onClick={onClick}
      className="group bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-all cursor-pointer p-6"
    >
      <div className="flex items-start justify-between mb-3">
        ...
        >
          <div className="relative group/menu">
            <button className="p-1 rounded-lg text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:text-slate-400 dark:text-slate-500 hover:bg-slate-100 dark:bg-slate-800 transition-colors">
              <MoreVertical className="w-4 h-4" />
            </button>
            <div className="absolute right-0 top-7 bg-white dark:bg-slate-900 border border-slate-100 shadow-xl rounded-xl py-1.5 z-10 w-36 hidden group-hover/menu:block">
              <button
                onClick={onEdit}
      ...
      <h3 className="font-bold text-slate-900 dark:text-slate-100 text-base mb-1 leading-tight">{project.name}</h3>
      <p className="text-sm text-slate-500 dark:text-slate-400 dark:text-slate-500 line-clamp-2 mb-4">{project.description}</p>
