    ...
    <div className="relative" ref={popoverRef}>
      ...
      <div className="flex flex-wrap gap-2 items-center">
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-semibold border border-dashed border-slate-300 text-slate-500 dark:text-slate-400 dark:text-slate-500 hover:text-indigo-600 hover:border-indigo-300 hover:bg-indigo-50 transition-all"
        >
      ...
      {/* Popover */}
      {isOpen && (
        <div className="absolute top-full left-0 mt-2 w-72 bg-white dark:bg-slate-900 rounded-xl shadow-xl border border-slate-200 z-50 overflow-hidden transform origin-top-left transition-all">
          <div className="p-3 border-b border-slate-100 bg-slate-50 dark:bg-slate-800/50/50 flex items-center justify-between">
            ...
            {(isCreating || editingTagId) && (
              <button 
                ...
                className="text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:text-slate-400 dark:text-slate-500"
              >
                <X className="w-4 h-4" />
              </button>
            )}
          ...
          <div className="p-3">
            {isCreating || editingTagId ? (
              <div className="space-y-4">
                <div>
                  <label className="text-[10px] font-bold uppercase text-slate-400 dark:text-slate-500 block mb-1.5">Nome</label>
                  <input
                    autoFocus
                    type="text"
                    value={newTagName}
                ...
                <div>
                  <label className="text-[10px] font-bold uppercase text-slate-400 dark:text-slate-500 block mb-2">Cor</label>
              ...
              <div className="space-y-2">
                <div className="max-h-48 overflow-y-auto space-y-1.5 pr-1">
                  {availableTags.length === 0 ? (
                    <p className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-500 text-center py-4">Nenhuma etiqueta disponível</p>
                  ) : (
                    availableTags.map(tag => (
                      ...
                      >
                        <div className="flex-1 flex items-center gap-2 p-1.5 hover:bg-slate-50 dark:bg-slate-800/50 rounded-lg transition-colors">
                        ...
                        <button
                          ...
                          className="opacity-0 group-hover:opacity-100 p-1.5 text-slate-400 dark:text-slate-500 hover:text-slate-700 dark:text-slate-300 transition-all rounded-md hover:bg-slate-100 dark:bg-slate-800"
                        >
                ...
                <button
                  ...
                  className="w-full mt-2 py-2 border-t border-slate-100 text-indigo-600 text-xs font-bold hover:bg-slate-50 dark:bg-slate-800/50 rounded-b-lg transition-colors flex items-center justify-center gap-1.5"
                >
